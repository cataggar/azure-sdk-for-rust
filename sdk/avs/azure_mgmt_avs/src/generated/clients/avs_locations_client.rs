...
impl AVSLocationsClient {
    ...
    pub async fn check_quota_availability(&self, location: String, options: Option<AVSLocationsClientCheckQuotaAvailabilityOptions<'_>>) -> Result<Response<Quota>> {
    ...
    pub async fn check_trial_availability(&self, location: String, options: Option<AVSLocationsClientCheckTrialAvailabilityOptions<'_>>) -> Result<Response<Trial>> {
        let options = options.unwrap_or_default();
        let ctx = Context::with_context(&options.method_options.context);
        let mut url = self.endpoint.clone();
