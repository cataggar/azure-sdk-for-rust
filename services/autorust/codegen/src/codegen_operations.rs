...
use autorust_openapi::{CollectionFormat, Header, ParameterIn, Response, StatusCode};
...
impl WebOperationGen {
    ...
    pub fn has_api_version(&self) -> bool {
        self.0.parameters.iter().any(|p| p.name() == API_VERSION)
    }

    #[allow(dead_code)]
    pub fn has_x_ms_version(&self) -> bool {
        self.0.parameters.iter().any(|p| p.name() == X_MS_VERSION)
    }
...
impl FunctionParams {
    fn new(operation: &WebOperationGen) -> Result<Self> {
        ...
        for param in parameters.iter().filter(|p| !skip.contains(p.name())) {
            ...
            let kind = ParamKind::from(param.parameter_in());
