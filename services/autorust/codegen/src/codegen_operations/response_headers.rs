        ...
        let hdr_fn = if type_name_code.is_string() {
            ...
            quote! {
                pub fn #function_name(&self) -> azure_core::Result<::time::OffsetDateTime> {
                    azure_openapi_core::date::parse_rfc3339(self.0.get_str(&azure_core::http::headers::HeaderName::from_static(#header_name))?)
                }
            }
        } else if type_name_code.is_date_time_rfc1123() {
            quote! {
                pub fn #function_name(&self) -> azure_core::Result<::time::OffsetDateTime> {
                    azure_openapi_core::date::parse_rfc1123(self.0.get_str(&azure_core::http::headers::HeaderName::from_static(#header_name))?)
                }
            }
        } else {
            quote! {
