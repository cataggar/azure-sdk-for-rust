...
pub fn type_name_gen(type_name: &TypeName, add_into: bool, qualify_models: bool) -> Result<TypePathCode, Error> {
    Ok(match type_name {
        TypeName::Reference(name) => {
            ...
            TypePathCode::from(idt).with_add_into(add_into)
        }
        TypeName::Array(vec_items_typ) => type_name_gen(vec_items_typ, add_into, qualify_models)?,
        TypeName::Value => TypePathCode::from(tp_json_value()).with_add_into(add_into),
        TypeName::Bytes => TypePathCode::from(tp_bytes()).with_add_into(add_into),
        TypeName::Int32 => TypePathCode::from(tp_i32()),
        TypeName::Int64 => TypePathCode::from(tp_i64()),
        TypeName::Float32 => TypePathCode::from(tp_f32()),
        TypeName::Float64 => TypePathCode::from(tp_f64()),
        TypeName::Boolean => TypePathCode::from(tp_bool()),
        TypeName::String => TypePathCode::from(tp_string()).with_add_into(add_into),
    })
}
...
impl From<Vec<Ident>> for TypePathCode {
    fn from(value: Vec<Ident>) -> Self {
        Self::from(idents_to_type_path(value))
    }
}

impl From<Ident> for TypePathCode {
    fn from(value: Ident) -> Self {
        Self::from(idents_to_type_path(vec![value]))
    }
}

impl From<Vec<Option<&Ident>>> for TypePathCode {
